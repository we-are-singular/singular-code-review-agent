...
function reviewContext(options) {
  const triggerComment = options.reason === "mention"
  ...
  return createEmptyReviewContext({
    ...
    issue_comments: triggerComment
      ? [{ id: 123, user: { login: "octocat" }, body: triggerComment.body, html_url: null, author_association: "MEMBER" }]
      : [],
  });
}
...
test("gate decision parser accepts only answer-bearing no-review and answer shapes", () => {
  ...
  assert.throws(() => parseGateDecision('{"decision":"answer","answer":"Yes.","reason":"extra"}'), /decision and answer/u);
});
...
test("rebase-equivalent force push sends range comparison to the gate", () => {
  const { repo, base, reviewed, defaultBranch } = createRepo();
  ...
  const result = prepareGate({ context, workspace: repo, diffText: git(repo, ["diff", `${newBase}..${head}`]), botLogin });
