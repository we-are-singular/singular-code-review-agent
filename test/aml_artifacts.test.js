...
import { writeAmlArtifacts } from "../eval/lib/aml-artifacts.mjs"
...
test("eval adapter writes four canonical artifacts from one in-memory result", () => {
  ...
  const exported = writeAmlArtifacts(result(), outputDir, "2026-08-23T00:00:00.000Z")
...
test("eval adapter replaces stale canonical artifacts", () => {
  const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "aml-artifacts-"))
  const first = writeAmlArtifacts(result(), outputDir, "2026-08-23T00:00:00.000Z")
  const secondResult = { ...result(), body: "✅ LGTM", payload: { ...result().payload, body: "✅ LGTM" } }
  writeAmlArtifacts(secondResult, outputDir, "2026-08-23T00:01:00.000Z")
