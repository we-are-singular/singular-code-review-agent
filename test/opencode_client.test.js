...
test("audit and synthesis prompts stay phase-specific because auditor owns post-processing scope", () => {
  ...
  assert.match(synthesisPrompt, /about 300 words/u)
  assert.match(synthesisPrompt, /Do not reproduce inline comments point by point/u)
  assert.match(synthesisPrompt, /representative examples only when/u)
  assert.match(synthesisPrompt, /When comments are unrelated, give a concise overall direction/u)
  assert.match(synthesisPrompt, /representative detail is allowed/u)
