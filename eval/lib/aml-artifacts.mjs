...
/** Normalizes AML's result into the existing judge's comment-export contract. */
...
/** Keeps the full typed AML evidence inspectable without making it workflow state. */
...
/** Converts one captured AML result into the compatibility artifacts used by eval/judge. */
export function writeAmlArtifacts(result, outputDir, generatedAt = result.generatedAt) {
  if (!result || typeof result !== "object") {
    throw new TypeError("AML result must be an object")
  }
...
/** Reads the single JSON result emitted by aml_review from its captured stdout. */
export function readAmlResult(stdoutFile) {
  if (!existsSync(stdoutFile)) {
    throw new Error(`AML stdout capture is missing: ${stdoutFile}`)
  }
  ...
  if (!payload) {
    throw new Error("AML stdout capture is empty")
  }
