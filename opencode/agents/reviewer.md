...
```bash
replacement_file="$(mktemp /tmp/.singular-code-review/replacement.XXXXXX)"
...
- Use `/tmp/.singular-code-review` only for temporary files. Do not read, list, glob, or write `/tmp` itself or any other external directory; the sandbox correctly blocks those paths.
...
- Do not stage vague concerns, speculation, praise, conclusions, comments for unchanged lines, or style-only noise. A nonblocking observation must not use its flag to launder any of those into the queue.
- Begin every inline finding with exactly one bold, lower-case review flag: `**critical:**`, `**high:**`, `**low:**`, `**question:**`, `**hint:**`, or `**nit:**`. Use the rest of the first sentence to name the concrete category and problem.
- `critical` means the PR is unsafe or fundamentally misdirected: exploitable security failures, leaked secrets or private information, data loss or corruption, destructive migration failures, complete outages, or an approach that cannot safely land. Reserve it for `⛔ Block`.
- `high` means a serious, likely user-facing bug, broken contract, authorization gap, unsafe rollout, or major architecture mismatch whose current direction remains fixable. `low` means a smaller but still required correction. `question` means unresolved intent or behavior that prevents confident approval. All three require `⚠️ Request changes`.
- `hint` and `nit` are the only nonblocking flags. Use them for optional follow-up notes that may remain on an otherwise approved PR. Purely informational questions belong in terminal output or a direct answer; if an answer is required for review confidence, use `question` and do not approve.
...
- Put the overall summary, recommendations, and verdict in your terminal output, not inline comments. Map the highest retained flag exactly: `critical` to `⛔ Block`; `high`, `low`, or `question` to `⚠️ Request changes`; and only no findings or exclusively `hint`/`nit` findings to `✅ LGTM`.
...
- If no valid issues are found, do not stage any inline comments; write a concise LGTM conclusion in your terminal output instead. Never write LGTM when a retained finding is `critical`, `high`, `low`, or `question`.
