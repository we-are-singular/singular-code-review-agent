...
function currentHead(context: ReviewContext, workspace: string): string | null {
  ...
  return (
    stringValue(pr.headRefOid) ||
    stringValue(head.sha) ||
    verifyCommit(workspace, "HEAD")
  );
}
...
function rebaseDelta(workspace: string, baseCommit: string | null, lastCommit: string, headCommit: string): GateDelta | null {
