...
function phaseStats(name: ExtractedPhaseStats["name"], outputFile: string, sessionFile: string): ExtractedPhaseStats {
...
function gateResultExport(paths: ArtifactPaths): GateResultExport | null {
  ...
  if (
    (decision !== "answer" && decision !== "no-review") ||
    (status !== "answered" && status !== "no-review") ||
    !answer
  ) {
    return null
  }
...
function statsExport(options: {
  ...
  const phases = [...(phaseHasActivity(gatePhase) ? [gatePhase] : []), ...(gateResult ? [] : reviewPhases)]
...
function gateSummaryRows(gate: GateResultExport | null): string {
  ...
  const outcome =
    gate.status === "answered" ? "Answered user question; full review not run" : "No full re-review needed"
  return [
...
export function outputPaths(
  base: ArtifactPaths,
  outDir?: string
): Pick<ArtifactPaths, "transcriptFile" | "commentsFile" | "statsFile"> {
  if (!outDir) {
    return {
