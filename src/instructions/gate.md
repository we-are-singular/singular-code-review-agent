...
For `no-review`, describe only what changed since the previous review and the resulting readiness. Use one paragraph of one or two plain sentences, at most 80 words and 600 characters. Do not recap the pull request, inventory changed files or prior findings, or repeat detailed evidence that belongs in the existing inline threads. Leave out headings and the LGTM marker because the application supplies them deterministically.
