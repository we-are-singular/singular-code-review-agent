...
For `no-review`, use at most two sentences to state what changed and why a full review is unnecessary. Do not restate previous findings; link them instead. Do not emit a verdict heading or LGTM marker because the application appends `## Verdict` and `✅ LGTM` deterministically.
