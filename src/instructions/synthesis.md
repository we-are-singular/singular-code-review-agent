...
Return `summary` as one paragraph of plain, direct language around a thousand characters, fifteen hundred at most. Explain the behavioral change, what the implementation gets right, and overall readiness. Do not repeat implementation detail the inline findings already cover, and do not name, paraphrase, list, group, count, or recommend individual findings, or mention their files, symbols, severities, or mechanisms. For requested changes, a broad statement that retained feedback must be resolved is enough. Length is guidance, not a gate.
...
The application renders blockers and notes verbatim in their own sections before the verdict. Account for blockers' effect on readiness without repeating their mechanism or action; notes never change readiness, so let their section own them. Include no headings, verdicts, or process descriptions in either prose field.
