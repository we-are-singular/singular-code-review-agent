...
- Preserve each finding's leading review flag when tightening or merging comments. Never downgrade `critical`, `high`, `low`, or `question` to `hint` or `nit`; preserve valid `hint` and `nit` comments as explicitly nonblocking.
