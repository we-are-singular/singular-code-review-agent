...
export function buildArtifactPaths(env: NodeJS.ProcessEnv, workspace: string, runtimeDir?: string): ArtifactPaths {
  ...
  return {
    ...
    contextFile: env.REVIEW_VALIDATION_CONTEXT_FILE || join(resolvedRuntimeDir, "review_validation_context.json"),
    gateContextFile: env.GATE_MODEL_CONTEXT_FILE || join(resolvedRuntimeDir, "gate_model_context.json"),
    gateDeltaFile: env.GATE_DELTA_FILE || join(resolvedRuntimeDir, "gate_delta.diff"),
    ...
    gateOutputFile: env.OPENCODE_GATE_OUTPUT_FILE || join(resolvedRuntimeDir, "opencode_gate.log"),
