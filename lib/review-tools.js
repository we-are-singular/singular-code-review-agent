...
function existingBotFindingMatches(context) {
  ...
  const matches = {
    unresolvedLocations: new Set(),
    restLocations: new Set(),
  ...
  if (context.review_threads_available) {
    for (const thread of context.unresolved_bot_threads || []) {
      ...
      matches.unresolvedLocations.add(commentLocationKey(comment));
    }
    return matches;
  }
  ...
  for (const comment of context.review_comments || []) {
    ...
    matches.restLocations.add(commentLocationKey(comparable));
  }
...
function validateQueue(queue, context) {
  ...
  const lastQueuedLocation = new Map();
  const existingBotMatches = existingBotFindingMatches(context);

  (queue.inlineComments || []).forEach((original, index) => {
    try {
      lastQueuedLocation.set(commentLocationKey(normalizeInlineComment(original)), index);
    } catch {
      // Invalid comments are reported during normal validation below.
    }
  });

  for (const [index, original] of (queue.inlineComments || []).entries()) {
    ...
    if (lastQueuedLocation.get(locationKey) !== index) {
      dropped.push({ kind: "inline", item: original, reason: "superseded by a later queued comment for this line" });
      continue;
    }

    if (existingBotMatches.unresolvedLocations.has(locationKey)) {
      dropped.push({ kind: "inline", item: original, reason: "unresolved bot thread already exists for this line" });
      continue;
    }

    if (existingBotMatches.restLocations.has(locationKey)) {
      dropped.push({ kind: "inline", item: original, reason: "previous bot comment already exists for this line" });
      continue;
    }

    const restBodyKey = `${locationKey}\0${normalizeComparableBody(result.comment.body)}`;
    if (existingBotMatches.restBodyKeys.has(restBodyKey)) {
